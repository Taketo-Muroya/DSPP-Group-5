...
```{r}
# read in geocoded dataset
landfall <- read_csv("hurs_geocode.csv") %>%
  filter(!is.na(County))
```
...
```{r, fig.asp=0.8}
map <- subset(landfall, Record == "Landfall")
...
g1 <- ggplot() + 
  geom_polygon(data=us.states, aes(x=long, y=lat, group=group), 
               color = "grey", fill = NA) + 
  geom_point(data=map, aes(x=Lon ,y=Lat), size=map$Wind/20,
             alpha=map$Wind/500, color="blue") + 
  xlab("longitude") + ylab("latitude") + 
  coord_cartesian(xlim=c(-102, -65)) + 
  ggtitle("Hurricane Landfalls (1851-2018)") + theme_void()
g1
...
g2 <- ggplot() + 
  ...
  xlab("longitude") + ylab("latitude") + 
  coord_cartesian(xlim=c(-102, -65)) + 
  ggtitle("Clustering Hurricane Landfalls") + theme_void()
g2
